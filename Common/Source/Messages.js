...
function MessageListOptions()
{
   ...
   if ( subMenu && mrTable )
   {
      ...
      IF_TB3(uiAddArchiveButton(mrTable));

      if ( TB3O.pageSelector === "report_list" )
      {
         if ( TBO_SHOW_REP_DEL_TABLE === "1")
         {
            var content = $g(IIF_TB4(ID_CONTENT,ID_MID2));
      ...
      var intMRP = parseInt10(TBO_MSG_RPT_PRELOAD) + 1;
      if ( !isIntValid(intMRP) ) { intMRP = 1; }
      if ( intMRP > 5 ) { intMRP = 5; }
   
      if ( intMRP > 1 )
      {
         IIF_TB3(MsgRptPreload(mrTable, intMRP);)
      }
   }
}
...
function uiModifyReport(bPopup)
{
   ...
   if ( rptBody )
   {
      if ( TBO_SHOW_BATTLE_REPORT === "1" ) 
      {
         IF_TB3(battleReportV2(rptBody, bPopup ? "":"orig");)
      }
...
function uiModifyMsgPost()
{
   ...
   function sendMessage(event)
   {
      ...
      {
         var mF = __TEST__($xf("//form[@name='msg' or @action='nachrichten.php']"));
         if ( mF ) { mF.submit(); }
      }
   }
