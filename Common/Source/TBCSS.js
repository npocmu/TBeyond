...
{{
   ...
   'div#tbver a {font-size:8pt; font-weight:bold; color:' + ifEqual(TB3O.nLayoutType,0,'#00FF00',1,'#008000') + ';}' +
}},
{{
   ...
   (
      ( TB3O.ServerInfo.version > 4.0 ) ? 
        'div#tbver {font-size:11px; width:180px; z-index:auto;}' +
        '#tbver .tbExTime, #tbver .tbTVer {display:block; color: black;}' +
        '#tbver .tbExTime {margin-top: 5px;}'
        :
        '#tbver .tbTVer {border-left:1px solid; margin-' + docDir[0] + ': 5px; padding-' + docDir[0] + ': 5px;}'
   ) +
}})
...
IF_TB3({{
   ...
   '#trading_edit + #tb_unitsdest { position: relative; float: '+docDir[1]+'; border: 1px solid silver; margin-right: 65px; margin-top: -48px; width: 270px; }' + 
   '#trading_edit + #tb_unitsdest tr.tbInfo td { font-size:11px; }' + 

   'table#tb_unitstime {margin-top:15px;width:350px;}' +
...
M4_ECHO_ON
   ...
   (( TBO_SHOW_BLDBLINK === '1' ) ?  '.tbUpgNow {text-decoration: blink !important;}' : '') +
   ...
   //big icons styles
   acss += 
   '.tb3BI img   {width:70px; height:' + IIF_TB4('67',(( TB3O.T35 ) ? '67' : '100')) + 'px;}' +
   '.tb3BI       {float:' + docDir[0] + ';background-repeat:no-repeat;}' +
   IF_TB4('.tb3BI       {margin-top:10px;}' +)
   ...
   '#n9.tb3BI       {background-image: url(' + image['setup'] + ');position:relative;}' +
